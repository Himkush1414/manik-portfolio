 ...
 * Animated Footer
 *
 * A cinematic, reveal-on-scroll block: two source images are re-drawn as
 * live ASCII art on <canvas>, light up in little clusters around the cursor,
 * and drift with a soft parallax. When it scrolls into view the display
 * heading unmasks character-by-character and the ASCII "hands" glide in from
 * opposite edges to meet in the middle.
 *
 * Ported from the vanilla "LukeBaffait Animated Footer" (GSAP + canvas) into a
 * single, self-contained, prop-driven React component. The reveal is driven by
 * an IntersectionObserver and text is split in JSX.
 *
 * Adapted for this project: the upstream `next-themes` dependency is dropped —
 * colors are always supplied explicitly by the caller.
 */
...
export interface AnimatedFooterProps {
  ...
  asciiChars?: string;
  /** Color of the ASCII glyphs. */
  charColor?: string;
  /** Fill color of a highlighted (hovered) cell. */
  hoverColor?: string;
  /** Glyph color inside a highlighted cell. */
  ...
  /** Play the reveal when the block scrolls into view (else it shows immediately). Defaults to true. */
  revealOnScroll?: boolean;
  /**
   * Controlled reveal. When set, the block ignores its own scroll observer and
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-between">
        <div
          ref={leftWrapRef}
          className="relative w-1/2 min-w-[200px] will-change-transform"
