// / (the main site) — "Skill & Fun" section hero background, ported from
// /lab/lv9's own copy (see chat reply). A curtain of vertical strips
// tiled edge to edge, each independently randomised (width, resting
// tone, depth) so the field reads as folded fabric rather than a flat,
// uniform grid. As the cursor approaches, nearby strips physically
// ROTATE (around their own vertical axis) toward the cursor, and
// rotating is what reveals a darker face underneath — not a flat colour
// swap synced to a fake rotation.
//
...
// it loaded. Isolated to this section's own module graph only; nothing
// here is imported by (or affects) any other view on the site. See
// skillsfun.ts for how this pauses (setHeroActive(false)) the instant
// the flip to the figure section completes.
import * as THREE from 'three';

const canvas = document.getElementById('skillsfun-strip-field') as HTMLCanvasElement | null;
const container = document.getElementById('skillsfun-hero') as HTMLElement | null;

// called from main.ts once page 2 has fully taken over — a hidden
// [visibility:hidden] section wouldn't otherwise stop a JS-driven rAF
// loop the way it stops a CSS animation, so this is the explicit pause.
let active = true;
...
if (canvas && container) {
  ...
  // A plain window "resize" event never fires just because this section's
  // own #skillsfun-root ancestor flips from display:none to display:block
  // (lv6-transition.ts's view switch) — unlike lv9's own body-level
  // version of this file, this container can be zero-size at the moment
  // layout() first runs above (whenever Skill & Fun hasn't been opened
  // yet), which bails out of layout() immediately (see its width<=0
  // guard) and leaves 0 strips laid out forever, with nothing to ever
  // retry it. A ResizeObserver on the container itself does correctly
  // fire for exactly that 0x0 -> real-size transition (its border-box
  // genuinely changes), which a window-level listener can't see —
  // caught by the flip transition simply never completing once actually
  // tested inside this integration, not on lv9 itself.
