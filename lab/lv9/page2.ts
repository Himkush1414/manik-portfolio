// /lab/lv9 — "page 2", revealed by the hero's strip-flip (see main.ts's
// Move Next handler + lv9-strip-field.ts's triggerFlipReveal + its own
// white-blur entrance, wired in index.html/lv9.css). This is a faithful
// copy of lv8's page2.ts as it currently exists, minus the clock-dial
// code that belonged to lv8's About section — that section isn't part
// of this copy at all, so there's no DOM for it to attach to. Two
// pieces live here:
//
...
// NOTE: .lv9-page2__ambient-layer's opacity transition (lv9.css) is a
...
// ---------------------------------------------------------------
// Ambient gradient — two stacked layers (see lv9.css's
// .lv9-page2__ambient-layer), each running its own continuous "breathe"
...
// ---------------------------------------------------------------
const ambientA = document.getElementById('lv9-ambient-a');
const ambientB = document.getElementById('lv9-ambient-b');
...
async function startPortraitCycle() {
  const canvas = document.getElementById('lv9-portrait-canvas') as HTMLCanvasElement | null;
  if (!canvas) return;
...
// ---------------------------------------------------------------
// Inert top-bar menu — purely visual toggle, no navigation (lv9 is a
// standalone route, Home/About/Projects/Contact don't go anywhere from
// here, same as lv8).
// ---------------------------------------------------------------
function wireMenu() {
  const btn = document.getElementById('lv9-page2-menu-btn');
  const panel = document.getElementById('lv9-page2-menu-panel');
  if (!btn || !panel) return;
