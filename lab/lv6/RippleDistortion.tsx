...
void main() {
  ...
  if (uGrayscale > 0.001) {
    // Scoped to the ripple field so only the distorted area under the cursor
    // turns black-and-white; everything outside stays full colour.
    float gray = dot(color, vec3(0.2126, 0.7152, 0.0722));
    color = mix(color, vec3(gray), uGrayscale * clamp(amount * 1.4, 0.0, 1.0));
  }
...
const RippleDistortion = ({
  ...
  useEffect(() => {
    ...
    // Guard against browsers/GPUs where a WebGL context can't be created —
    // degrade to nothing rendered rather than throwing out of the effect.
    ...
    try {
      ...
      console.error('[RippleDistortion] WebGL unavailable:', err);
      return;
    }
    const gl = renderer.gl;
    if (!gl) {
      console.error('[RippleDistortion] no WebGL context');
      return;
    }
    ...
    image.onerror = () => console.error('[RippleDistortion] texture failed to load:', src);
    image.src = src;
    ...
    const setNewWave = (x: number, y: number, power: number) => {
      ...
      // Was hardcoded to 1 — now carries the same power/fade factor as the
      // size does, so a wave born near/past the mount's own edge (see
      // localPointWithFade below) is fainter as well as smaller, rather
      // than full-strength right up until the boundary and then abruptly
      // absent past it. min(1, ...) keeps click's stronger clickStrength
      // power (can be >1) from over-brightening — clicks still start at
      // full opacity exactly as before, only hover's edge-fade actually
      // varies this.
      wave.opacity = Math.min(1, power);
    };
    ...
    // How far beyond the mount's own box (css px) the hover effect keeps
    // responding at all, fading out smoothly to nothing rather than the
    // hard binary cutoff localPoint() gives right at the box's exact edge
    // — which reads as a visible boundary line the instant the cursor
    // crosses it, on any side (this was most noticeable moving the cursor
    // upward, away from the watermark, but the box has all four edges).
    // Sized roughly to the effect's own default brush reach so the fade
    // zone feels like a natural continuation of the ripple, not an
    // arbitrary strip tacked on around it.
    const EDGE_FADE = 220;

    const localPointWithFade = (clientX: number, clientY: number): [number, number, number] | null => {
      const rect = mount.getBoundingClientRect();
      if (rect.width === 0 || rect.height === 0) return null;
      ...
      if (dist >= EDGE_FADE) return null;

      // 1 at/inside the box, smoothly down to 0 at EDGE_FADE px past it —
      // smoothstep rather than linear so the taper itself has no hard
      // kink at either end (full-strength start or zero-strength end).
      const t = 1 - dist / EDGE_FADE;
      const power = t * t * (3 - 2 * t);

      // The wave still has to land somewhere the canvas can actually draw
      // it — clamp to the nearest point on the box's own edge; `power`
      // (fed into setNewWave below) is what makes a wave born out here
      // read as fading, not its clamped position.
      ...
      return [clampedX - rect.left, rect.height - (clampedY - rect.top), power];
    };
    ...
    const onMove = (event: PointerEvent) => {
      ...
      const point = localPointWithFade(event.clientX, event.clientY);
      if (!point) return;
      const [x, y, power] = point;
